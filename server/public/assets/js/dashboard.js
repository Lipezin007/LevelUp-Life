...
async function completeActivity(activityId, opts) {
  ...
  if (!activity) throw new Error("Atividade não existe");
...
function showXpModal({ totalXP, gains }) {
  ...
  text.innerHTML = `<strong>Total:</strong> ${totalXP} XP<br><br>${lines || "Cap diário atingido"}`;
...
function renderLog() {
  ...
  for (const item of state.log) {
    ...
    const gainsTxt =
      Object.entries(item.gains)
        .filter(([, v]) => v > 0)
        .map(([k, v]) => `${SKILL_DEFS[k].name}: +${v}`)
        .join(" • ") || "(cap diário atingido)";

    const div = document.createElement("div");
    div.className = "log-item";
    div.innerHTML = `
      <strong>${a?.name ?? item.activityId}</strong>
      <div class="muted">${date.toLocaleString()} • ${item.minutes} min • ${item.difficulty} • sem distração: ${item.distractionFree ? "sim" : "não"}</div>
      <div>${gainsTxt}</div>
    `;
...
function renderQuests() {
  ...
  for (const q of quests) {
    const div = document.createElement("div");
    div.className = "quest";
    const right = q.type === "xp" ? `${Math.min(q.progress, q.target)}/${q.target} XP` : `${Math.min(q.progress, q.target)}/${q.target}`;

    div.innerHTML = `
      <div>
        <strong>${q.done ? "✅ " : "🎯 "}${q.text}</strong>
        <div class="muted">Progresso: ${right}</div>
      </div>
      <span class="pill">${q.done ? "Concluída" : "Em progresso"}</span>
    `;
...
function renderDayInfo() {
  ...
  const top = Object.entries(earned)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([k, v]) => `${SKILL_DEFS[k].name}: ${v}`)
    .join(" • ");
...
document.getElementById("btnComplete").addEventListener("click", async () => {
  ...
  const result = await completeActivity(activityId, { minutes, difficulty, distractionFree });
...
document.getElementById("btnReset").addEventListener("click", async () => {
  if (!confirm("Tem certeza? Isso apaga tudo e recomeça do zero.")) return;
  state = defaultState();
  await persistState(state);
  renderAll();
});
